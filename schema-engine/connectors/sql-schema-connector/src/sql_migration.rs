        ...
        for (idx, step) in self.steps.iter().enumerate() {
            ...
            match step {
                ...
                SqlMigrationStep::CreateExtension(create_extension) => {
                    let ext: &postgres::PostgresSchemaExt = self.schemas().next.downcast_connector_data();
                ...
                SqlMigrationStep::AlterExtension(alter_extension) => {
                    let ext: &postgres::PostgresSchemaExt = self.schemas().previous.downcast_connector_data();
